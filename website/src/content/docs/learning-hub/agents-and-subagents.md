...
Yes. As of v1.0.72+, **multi-turn subagents are always enabled** — you can send follow-up messages to a subagent while it is still running, just as you would in the main conversation. Previously, this was an opt-in feature; it is now on by default for all users.
